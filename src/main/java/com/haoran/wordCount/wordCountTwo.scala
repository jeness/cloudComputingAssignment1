...
////       val conf = new SparkConf().setAppName("WordCount").setMaster("spark://cluster-assign1-m:8032")  // 运行在集群中
//       val conf = new SparkConf() // 运行在集群中
//
//      /**
...
//        */
//      val lines = sc.textFile("gs://ufcloudcomputing/bible")   // 读取gs cloud文件
////      val lines = sc.textFile("bible")   // 读取本地文件
...
//      val wordsLength = (int)words.count();
//      var TwoWordsArray : Array[String] = new Array[String](wordsLength-1);
//      for(i <- 0 until TwoWordsArray.length-1){
////        println("i is: " + i);
////        println("i'th element is: " + myArray(i));
//        TwoWordsArray(i) = words(i) + " " + words(i+1);
//        
//      }
//      
...
//      wordscount.coalesce(1).saveAsTextFile("gs://ufcloudcomputing/wordcountOneOutput_oneFile") //save and merge result as one file
//      sc.stop()   // 释放资源
//  }
//}